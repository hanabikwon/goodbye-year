...
import { lightQuestions } from "@/data/questions";
...
export default function LightTestPage() {
  ...
  const handleAnswer = (answer: string) => {
    setAnswers((prev) => ({ ...prev, [lightQuestions[currentQuestion].id]: answer }));
  };

  const handleNext = () => {
    if (currentQuestion < lightQuestions.length - 1) {
      setCurrentQuestion((prev) => prev + 1);
    } else {
      const encoded = btoa(encodeURIComponent(JSON.stringify(answers)));
      router.push(`/test/light/result?data=${encoded}`);
    }
  };
  ...
  const currentQ = lightQuestions[currentQuestion];
          ...
          <div className="felt-card stitch-border p-6 mb-6">
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-[#5c4a3a]">변화</p>
                  <p className="text-sm text-[#8b7355]">3개 질문</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
            ...
            <div className="pt-4 border-t border-[#d4a574]/30 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-[#5c4a3a]">약 5분</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-[#8b7355]">AI 분석</span>
                <span className="text-[#6b8e6b]">2025년 총평 + 감정 분석</span>
              </div>
            </div>
          </div>
  ...
  return (
    <main className="min-h-screen flex flex-col items-center justify-center p-6">
      ...
      <QuestionCard
        ...
        totalSteps={lightQuestions.length}
      />
      ...
      <div className="flex gap-4 mt-8">
        ...
        >
          {currentQuestion === lightQuestions.length - 1 ? "완료" : "다음"}
        </button>
      </div>
    </main>
  );
